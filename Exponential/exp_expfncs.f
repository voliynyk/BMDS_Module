...
c               to the exponential models when doing maximum likelihood and confidence
...
c       I: 6
           ...
           GUNIT(2, J) = 5
	   GUNIT(3, J) = 1
C       c parameter upper bound if adverse direction down; I: 7
...
c       
		...
		ENDDO
           ELSE
		DO I = 1, ndoses
                means(I) = X(3)*(X(5)-(X(5)-1)*EXP(-1.0*(X(4)*doses(I))**X(6)))
		ENDDO
           ENDIF
           	varmean = 0.0
           DO I = 1, ndoses
...
c       
c       
c       EXPpart gives the partial derivatives of the exponential model mean function
...
C       INCLUDE 'O8COMM.INC'
        ...
        EPS = 1.0d-8
c       
c       Maximum likelihood estimation exponential model mean partial derivatives
        IF(probtype.EQ.1) THEN
           DO I = 1, ndoses
              grads(I,1) = 0.0
              grads(I,2) = 0.0
		 ...
		 grads(I,3) = EXP(adverse*(doses(I)*X(4))**X(6))
                 grads(I,4) = adverse*doses(I)*EXP(adverse*(doses(I)*X(4))**X(6))*X(3)*(doses(I)*X(4))**(X(6)-1)*X(6)
                 grads(I,5) = 0.0
		 grads(I,6) = adverse*EXP(adverse*(doses(I)*X(4))**X(6))*X(3)*(doses(I)*X(4))**X(6)*LOG(doses(I)*X(4))
              ELSE
		 grads(I,3) = (X(5)-(X(5)-1)*EXP(-1.0*(X(4)*doses(I))**X(6)))
                 grads(I,4) = X(3)*(X(5)-1)*EXP(-1.0*(X(4)*doses(I))**X(6))*(doses(I)*X(4))**(X(6)-1)*X(6)
                 grads(I,5) = X(3)*(1.0-EXP(-1.0*(X(4)*doses(I))**X(6)))
		 grads(I,6) = X(3)*(X(5)-1)*EXP(-1.0*(X(4)*doses(I))**X(6))*(doses(I)*X(4))**(X(6))*LOG(doses(I)*X(4))
              ENDIF
           ENDDO
...
c	c > 1 or > 0 (adverse up or down, respectively)
	   ELSEIF (I .EQ. 6) then 
              IF (adverse .EQ. 1) THEN            
                 GXI = X(GUNIT(2, I+NH)) - (1 + EPS)
              ELSE 
                 GXI = X(GUNIT(2, I+NH)) - EPS
              ENDIF
c       c < 1 if adverse = -1 (there will be no I=7 case unless adverse = -1: from gunitfill above)
           ELSEIF (I .EQ. 7) THEN 
              GXI = (1 - EPS) - X(GUNIT(2, I+NH)) 
           ENDIF
