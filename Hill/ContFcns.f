...
c       
	...
	CALL EstVar(X,Vi)
	DO I = 1,ndoses
	   IF(Vi(I).LE.0) Vi(I) = .00000001
	   Ni = nanimals(I)
	   SUM = SUM + Ni*DLOG(Vi(I))/2.0D0 + 
     1	   (Ni-1)*var(I)/(2.0D0*Vi(I))
     2   + Ni*((mean(I)-means(I))**2)/(2.0D0*Vi(I))
	ENDDO
	LK = SUM
c       
	RETURN
...
c       
	...
	INCLUDE 'PROBLEM.INC'
	INTEGER I,ptype
	DOUBLE PRECISION Vi(*),X(*), DLOG
c
c       When probtype .eq 2, the first parameter is the BMDL, which
c       is absent in the other two cases
	...
	DO I = 1,ndoses
	   IF(constvar.EQ.1) THEN
	      Vi(I) = X(1+ptype)
	   ELSE
	      Vi(I) = DEXP(X(1+ptype) + DLOG(DABS(means(I)))*X(2+ptype))
	   ENDIF
	ENDDO
	RETURN
	END
...
c	have been called prior to invoking this routine
	...
	INCLUDE 'PROBLEM.INC'
	INTEGER I,ptype
	DOUBLE PRECISION Vi(MAXDOSES),DABS,dFi1,dFi2,dFi3
...
c	Get estimated variance
	...
	DO I = 1,ndoses
	   ...
	   DO J = 1+ptype,nparm
	      ...
	      dFi3 = dFi3/(Vi(I)*Vi(I))
	      TEMP = Ni*dFi1/2.0D0 + (Ni-1)*var(I)*dFi2/2.0D0 + Ni*dFi3/2.0D0
	      G(J) = G(J) + TEMP
	   ENDDO
	   IF(ptype.EQ.1) THEN
	      G(1) = 0.0
...
c	Upon return to calling function, DVi(I,J) = dV/dX(J) at dose I
	...
	DOUBLE PRECISION Vi(*),DVi(MAXDOSES,*),DLOG,X(*)
c       
	ptype = probtype - 1
	IF(probtype.EQ.3) ptype = 0
	sign = 1
...
c       
	   ...
	   ELSE
		 ...
		 DVi(I,2+ptype) = 0.0
	      ELSE
		 DVi(I,2+ptype) = Vi(I)*LOG(ABS(means(I)))
	      ENDIF
...
c       
	...
	IF(bmrtype.EQ.0) THEN
	   ...
	   G(3) = 0.0
	   G(4) = 0.0
	   G(5) = sign*DoseMeanGrad(5)
 	   G(6) = sign*DoseMeanGrad(6)
	   G(7) = sign*DoseMeanGrad(7)
	ELSEIF(bmrtype.EQ.1) THEN
	   IF(X(4).LT.0) THEN
	      sign2 = -1
	   ELSE
	   ...
	   G(5) = sign*DoseMeanGrad(5)/STD
	   G(6) = sign*DoseMeanGrad(6)/STD
	   G(7) = sign*DoseMeanGrad(7)/STD
	ELSEIF(bmrtype.EQ.2) THEN
	   G(1) = sign*DoseMeanGrad(1)/X(4)
	   G(2) = 0.0
	   G(3) = 0.0
	   G(4) = -sign*MeanDev/(X(4)*X(4))
	   G(5) = sign*DoseMeanGrad(5)/X(4)
	   G(6) = sign*DoseMeanGrad(6)/X(4)
	   G(7) = sign*DoseMeanGrad(7)/X(4)
	ELSEIF(bmrtype.EQ.3) THEN
	   G(1) = DoseMeanGrad(1)
	   G(2) = 0.0
	   G(3) = 0.0
	   ...
	   G(7) = DoseMeanGrad(7)
	ELSE
	   G(1) = DoseMeanGrad(1)/X(5)
	   G(2) = 0.0
	   G(3) = 0.0
	   G(4) = 0.0
	   G(5) = DoseMeanGrad(5)*X(5)-MeanDev
	   G(5) = G(5)/(X(5)*X(5))
	   G(6) = DoseMeanGrad(6)/X(5)
	   G(7) = DoseMeanGrad(7)/X(5)
	ENDIF
	RETURN
	END
