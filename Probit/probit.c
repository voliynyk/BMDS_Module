...
*
*****************************************************************/
char Version_no[]="Probit Model. (Version: 3.3;  Date: 2/28/2013)";
/****************************************************************
char Version_no[]="Probit Model. (Version: 3.2;  Date: 10/28/2009)";
char Version_no[]="Probit Model. (Version: 3.1;  Date: 05/16/2008)";
char Version_no[]="Probit Model. (Version: 3.0;  Date: 03/12/2008)";
char Version_no[]="Probit Model. (Version: 2.9;  Date: 09/23/2007)";
char Version_no[]="Probit Model. (Version: 2.8;  Date: 02/20/2007)";
char Version_no[]="Probit Model. (Version: 2.6;  Date: 03/22/2006)";
*****************************************************************/
/****************************************************************
**
* Probit.C - a ANSI C program for Probit model fitting with/without
...
* Reason: Commented out cancer slope factor.
*
* Version Number: 2.5
* Modified By: R. Woodrow Setzer
* Modified Date: 09/25/2005
* Reason: free all allocated memory before exit
*
* Version Number: 2.6
* Modified By: R. Woodrow Setzer
...
void Probit_fit (int nparm, double *p, double gtol, int *iter, double *fret);
void Probit_vcv (int nparm, int *Spec, double *p, double **vcv);
void Which_Bounded (int [], double [], int []);
int Model_DF (int []);
void Probit_BMD (int nparm, double *p, double gtol, int *iter, double xlk,
...
/****************************************************************
** main--main function used to call Probit mode fitting program.
...
int main (int argc, char *argv[])
{
	...
	*********************************************************************/
	...
	if (logtrans==No)
	{
		if (fixedParm(Background))		/* check user specified values */
		{
			...
			{
				Parms[(int) Intercept]= RNORM(0.00005);
			}
			else
			{
				Parms[(int) Intercept ]= RNORM(Parms[(int) Background]);
			}
			Spec[(int) Intercept ]=1;
		}
		/* end if (Spec[(int) Background] ==1) */
	...
	{
		FreeUp_mem(Parms, varsum, anasum, vcv);
		ERRORPRT("Observation # < parameter # for Probit model.");
	}
	...
	if(Xi[1] == 0.0 && fixedParm(Background) && Parms[(int) Background] == 0.0 &&
		Yp[1] != 0 && logtrans != No)	//Prefix condition with "Xi[1] == 0.0 &&" {GLN - 09/23/07, PR0823-05}
	{
		FreeUp_mem(Parms, varsum, anasum, vcv);
	...
	{
		fprintf(fp_out,"\n   Intercept parameter is set to %g", Parms[2]);
	}

	if (fixedParm(Slope))		/* if slope parameter is specified */
	{
		fprintf(fp_out,"\n   Slope parameter is set to %g", Parms[3]);
	}
	else
	{
	...
	*  lkf and lkr are the log-likelihood functions for the binomial dist*/
	...
	/* fitting Probit model and output parameter estimators */
	Probit_fit(nparm, Parms, EPS, &iter, &xlk);
	...
	if (ErrorFlag == 0)
	{
		...
		/* Compute the Hessian (in Probit_vcv) for ALL parameters, */
		...
		Probit_vcv(nparm,Spec,Parms,vcv);
	...
	if (bmdose==Yes)
	{
		...
		{
			back=CNORM(Parms[(int) Intercept]);
		}
		...
		Probit_BMD (nparm, Parms, EPS, &junk, xlk, Rlevel, Bmdl, &BMD);
...
void Predict(double doses[], int ndoses, double Parms[], double P[])
{
	...
	{
		for (i = 1; i <= ndoses; i++)
			P[i] = CNORM(Parms[(int) Intercept] + Parms[(int) Slope] * doses[i]);
	}
	else
	{
		for (i = 1; i <= ndoses; i++)
			if (doses[i] <= 0.0)
				...
				CNORM(Parms[(int) Intercept] + Parms[(int) Slope] * log(doses[i]));
	}
}
...
void unpack(double x[], double fixed[], double p[])
{
	...
	if (replace==Yes)
	{
		if(logtrans==Yes) /* replace p[(int) Intercept]; tD is already on the log scale */
		{
			...
			{
				p[(int) Intercept] = RNORM(BMR/(1.0 - p[(int) Background])) - p[(int) Slope] * tD;
			}
			else
			{
				p[(int) Intercept] = RNORM(BMR) - p[(int) Slope] * tD;
			}
		}
		else /* replace p[(int) Slope]; not log dose */
		{
			...
			{
				Z = BMR*(1.0 - CNORM(p[(int) Intercept]));
			}
			p[(int) Slope] = (RNORM(Z + CNORM(p[(int) Intercept])) - p[(int) Intercept])/tD;
		}
	}
}
...
void Which_Bounded (int Spec[], double Parms[], int bounded[])
{
	...
	{
		bounded[1] = 1;
		if (!fixedParm(Slope) && VERYCLOSE(Parms[3],(log(Max_double)/xmax)))
			bounded[3] = 1;
	}
	else /* logtrans == Yes */
	{
...
/*******************************************************************
**Probit_lk -- used to compute the (minus) log likelihood for Probit model.
...
void Probit_lk(long int *nvar, double *x, long int *nf, double *f,
...
/*******************************************************************
**Probit_g -- used to compute the gradients for Probit model.
*		Extern var.: smean, smax, Nobs, Xi, Yp, Yn, Ls, Xg.
*
*		input:
*			nvar is the number of parameters
...
void Probit_g (long int *nvar, double *x, long int *nf, double *g,
			   long int *uiparm, double *urparm, void (*ufparm) ())
{
	...
	for (i = 0; i < *nvar; i++)
	{
		...
		saveparms[i] = x[i] - h[i];
		Probit_lk(nvar, saveparms, &inf, &basefun, uiparm, urparm, ufparm);
		saveparms[i] = x[i] + h[i];
		Probit_lk(nvar, saveparms, &inf, &tmp, uiparm, urparm, ufparm);
...
/****************************************************************************
 **
 * Probit_grad -- Computes the gradient of the Probit likelihood
 * function with respect to the user form of the parameters.  This is to
 * be used in Probit_vcv, to compute a finite difference approximation to
 ...
 * Output: grad[] -- the gradient of the loglikelihood function
 (N.B. Probit_g returns the gradient of -loglikelihood)
 Based at 1.

 *****************************************************************************/
void Probit_grad(int nparm, int Spec[], double ptf[],
				 double grad[])
{
	...
	replace = No;
	Probit_g (&nvar, start, &nf, outgrad, uiparm, urparm, ufparm);
	/* Probit_g returns the gradient of -loglikelihood */
	for (i = 0; i < nvar; i++)
	{
		grad[i+1] = -outgrad[i];
	}
...

/****************************************************************************
*	Probit_vcv -- used to compute the vcv matrix for Probit model.
*
...
void Probit_vcv (int nparm, int Spec[], double ptf[], double **vcv)
{
	...
	for (i = 1; i <= nparm; i++)
	{
		...
		if (Spec[i] != Yes)
		{
			...
			Probit_grad(nparm, Spec, saveparms, gradp);
			saveparms[i] = ptf[i] - h[i];
			Probit_grad(nparm, Spec, saveparms, gradm);
...

{
	...
	else
	{
		...
		Probit_lk( &nvar, start,  &dummy, fret,
	...
	ErrorFlag = run_dmngb((int) nvar, start, lower, upper, Maxloglik,
		...
		Probit_lk,Probit_g,uiparm,urparm,ufparm,
		0,fret);
	*fret = -*fret;
...
/**************************************************************
*Probit_fit -- Used to "prepare" the data for further computation,
*            i.e. compute the extern variables, give the initial
*            parameters, etc. THEN fit the Probit model.
...
*
***************************************************************/
void Probit_fit(int nparm, double p[], double gtol,
				int *iter, double *fret)
{
	...
	/* For normal Probit model, scale dose. */
	...
	else
	{
		...
		if (logtrans == Yes)
		{
			...
			for (i=1;i<=Nobs;i++)
			{
				...
				dos=log(Xi[i]);
				vcv[1][1] += Yp[i] + Yn[i];
				vcv[1][2] += dos * (Yp[i] + Yn[i]);
				vcv[2][2] += dos * dos *(Yp[i] + Yn[i]);
				junk1 += rw * (Yp[i] + Yn[i]);
				junk2 += dos * rw * (Yp[i] + Yn[i]);
			}
		...
		else /* logtrans == No */
		{
			...
			for (i=1;i<=Nobs;i++)
			{
				...
				vcv[1][2] += dos * (Yp[i] + Yn[i]);
				vcv[2][2] += dos * dos * (Yp[i] + Yn[i]);
				junk1 += (tr * Yp[i]) + (tl * Yn[i]);
				junk2 += dos * ((tr * Yp[i]) + (tl * Yn[i]));
			}
...
/************************************************************
 * Probit_BMD -- Used to calculate the BMD and BMDL for Probit model.
 *
 *		Input:
	...
	if (p[(int) Slope] <= DBL_MIN)
	{
		*BMD = 100 * xmax;
		...
		if(logtrans==Yes)
		{
			if (bmdparm.risk == ADDED)
			{
				xb = (RNORM(BMR/(1.0 - p[(int) Background])) - p[(int) Intercept]) / p[(int) Slope];
			}
			else
			{
				xb = (RNORM(BMR) - p[(int) Intercept]) / p[(int) Slope];
			}
			*BMD = exp(xb);
		}
		else
		{
			...
			*BMD = (RNORM(Z + CNORM(p[(int) Intercept])) - p[(int) Intercept])/p[(int) Slope];
			xb = *BMD;
		}
		...
		/* Replace either the slope (ordinary Probit) or the intercept
		(log-Probit) with an expression involving the BMD */
		...
		while (fa<0.0 &&
			((logtrans == No && xa > DBL_MIN) || (logtrans == Yes && exp(xa) > DBL_MIN)) && fabs(fa - fb) > DBL_EPSILON)
			...
			for (k=2; k<=5; k++)
			{
				...
				if (p[(int) Slope] <= DBL_MIN)
				{
					...
					else
					{
						...
						BMDtmp = (RNORM(Z + CNORM(p[(int) Intercept])) - p[(int) Intercept])/p[(int) Slope];
						xb = BMDtmp;
					}
					...
					while (fa<0.0 && ((logtrans == No && xa > DBL_MIN) || (logtrans == Yes && exp(xa) > DBL_MIN)) && fabs(fb - fa) > DBL_EPSILON) /* BMDL does not lie between xa and xb, so reduce xa */
					{
						xb = xa;
						fb = fa;
						for (i = 1; i <= nparm; i++) pb[i] = pa[i];
