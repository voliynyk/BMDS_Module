...
*
*****************************************************************/
//char Version_no[]="MS_COMBO. (Version: 1.0;  Date: 07/06/2007)";
//char Version_no[]="MS_COMBO. (Version: 1.1;  Date: 03/13/2009)";
//char Version_no[]="MS_COMBO. (Version: 1.2;  Date: 04/30/2009)";
char Version_no[]="MS_COMBO. (Version: 1.3;  Date: 10/28/2009)";
...
char    fout3[FLENGTH]; /* output file for logging */
...
double xmax, xmin, cxmax; /* max and min dose levels */
...
int main (int argc, char *argv[])
{
	...
	*  Code below moved outside of loop    */
	...
	for (inm=0;inm<FLENGTH && fout[inm] != '.'; inm++)
	...
	/* GLN-01/14/2009 */
	aDataList = (ObsDataList *) malloc((size_t) ((nT-1+1+NR_END)*sizeof(ObsDataList)));
	aParmList = (ParmListVar *) malloc((size_t) ((nT-1+1+NR_END)*sizeof(ParmListVar)));
	int nParmMax = 0;
	/* End GLN-01/14/2009 */
	...
	for (nt = 1; nt <= nT; nt++)
	{
		...
		/* End GLN-01/14/2009 {Allocate memory and initialized} */
		...
		/* --------------------------------------------------------------------------------------- */
		/* For now, set these options here in code.  The control whether to report the cancer slope factor, and
		whether to plot the linear extrapolation */
		ReportCancerSlopeFactor = 0;
		...
		/* End of GLN-01/23/2009 {Assign values} */
		...
		/* define vector of flags aParmList[nt].pnSpec[], 0=unknown, 1=specified */
		...
		brat=Yes;
		if (aParmList[nt].pnSpec[1]==1 && aParmList[nt].pdParms[1]<EPS) /* if background is spec. and small */
			brat=No;
					...
					int *iter, double *fret)
{
	...
	for(i = 1; i <= Nobs; i++)
	{
		nanim[i-1] = (int) (aDataList[nt].pdYp[i] + aDataList[nt].pdYn[i]); /* # of animals at ith dose level */
		doses[i-1] = aDataList[nt].pdXi[i]/xmax;                 /* dose levels; rescaled to: 0 <= Dose <= 1  */
		affect[i-1] = (int) (aDataList[nt].pdYp[i]);        /* # of affected animals at ith dose level */
	} /* end for */
