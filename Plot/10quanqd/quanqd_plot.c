...
/*********************************************************************
*
*  Quanqd_plot.c - an ANSI C program that reads the output file 
*                   from Quanqd.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
**********************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  if (argv[2] == NULL)
  ...
  else
    {
      ...
      fprintf(fp_out, "set output \"Quanqd.ps\" \n");
    }
  ...
  if (flag1 == 1 && flag2 == 1)
    {
      fprintf(fp_out, "set title 'Quantal Quadratic Model, with %lg%% BMR and %lg lower confidence limit for BMD (BMDL)' \n", rsl*100., conf);
      fprintf(fp_out, "rl = %g \n", rsl);
  ...
  fprintf(fp_out, "f(x)= back + (x>0?( (1- back)*(1-exp(-slope*x**2)) ) : 0) \n");
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=quanqd_plot.o EXEBASE=10quanqd" */
/* End: */
