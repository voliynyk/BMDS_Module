...
int main(int argc, char *argv[])
{
  ...
  if (argv[2] == NULL)
  ...
  else
    {
      ...
      fprintf(fp_out, "set output \"Gammhit.ps\" \n");
    }
  ...
  if (flag1 == 1 && flag2 == 1)
    {
      fprintf(fp_out, "set title 'Gamma Multi-Hit Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL'\n",
    ...
    fprintf(fp_out, "set title 'Gamma Multi-Hit Model' \n");

  fprintf(fp_out, "back = %g \n", back);
  fprintf(fp_out, "slope = %g \n", slope);
  fprintf(fp_out, "power = %g \n", power);
  fprintf(fp_out, "f(x)= back + (x>0? ( (1- back)*igamma(power,x*slope) ) : 0) \n");
  xrange=xmax-xmin;
  yrange=ymax-ymin;
  ...
  fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Gamma Multi-Hit',\\", 
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=gamm_plot.o EXEBASE=10gammhit" */
/* End: */
