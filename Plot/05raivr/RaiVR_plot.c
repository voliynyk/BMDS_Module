...
/*********************************************************************
*
*  RaiVR_plot.c - an ANSI C program that reads the output file 
*                   from NaiVR.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
**********************************************************************/
...
/*** Start to produce the output file ***/
  ...
  if (argv[2] == NULL)
  ...
  else
    {
      ...
      fprintf (fp_out, "set output \"RaiVR.ps\" \n");
    }
  ...
  if (flag1 == 1 && flag2 == 1)
    {
      fprintf (fp_out, "set title 'RaiVR Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
    ...
    fprintf (fp_out, "set title 'RaiVr Model' \n");
  ...
  fprintf (fp_out, "fixd = %g \n", fixedsize);
  fprintf (fp_out, "f(x)= (1-exp(-alpha-beta*x**rho))*exp(-fixd*(theta1+theta2*x)) \n");
  ...
  fprintf (fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'RaiVR',\\",
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=RaiVR_plot.o EXEBASE=05raivr" */
/* End: */
