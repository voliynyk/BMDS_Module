...
/*********************************************************************
*
*  Weibull_gnu2.c - an ANSI C program that reads the output file 
*                   from Weibull.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
**********************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  double  background, slope, power, conf;
  ...
  if (argv[2] == NULL)
  ...
  else
    {
      ...
      if (modeltype == 1)
	{
	  fprintf(fp_out, "set output \"Weibull.ps\" \n");
	}
      else if (modeltype == 2)
	{
	  fprintf(fp_out, "set output \"Quantal Linear.ps\" \n");
	}
      else
	{
	  fprintf(fp_out, "set output \"Quantal Quadratic.ps\" \n");
	}
    }
  ...
  if (flag1 == 1 && flag2 == 1)
    {
      if (modeltype == 1)
	{
	  fprintf(fp_out, "set title 'Weibull Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
		  effect*100., pcRiskType, conf);
	}
      else if (modeltype == 2)
	{
	  fprintf(fp_out, "set title 'Quantal Linear Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
		  effect*100., pcRiskType, conf);
	}
      else 
	{
	  fprintf(fp_out, "set title 'Quantal Quadratic Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
		  effect*100., pcRiskType, conf);
	}
  ...
  else
    {
      if (modeltype == 1)
	{
	  fprintf(fp_out, "set title 'Weibull Model' \n");
	}
      else if (modeltype == 2)
	{
	...
	{
	  fprintf(fp_out, "set title 'Quantal Quadratic Model' \n");
	}
    }
  fprintf(fp_out, "background = %lg \n", background);
  fprintf(fp_out, "slope = %lg \n", slope);
  fprintf(fp_out, "power = %lg \n", power);
  fprintf(fp_out, "f(x)= background+(x>0?( (1-background)*(1-exp(-slope*x**power)) ) : 0) \n");
  ...
  if (modeltype == 1)
    {
      fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Weibull',\\", 
    ...
    {
      fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Quantal Quadratic',\\", 
	      xmin>0?0:xmin, xmax, ybottom, ytop);
    }
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=weibull_plot.o EXEBASE=10weibull" */
/* End: */
