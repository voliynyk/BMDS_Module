...
/*********************************************************************
*
*  Nlogist_plot.c - an ANSI C program that reads the output file 
*                   from Nlogist.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
**********************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  char  name1[CNLENGTH], name2[CNLENGTH], name3[CNLENGTH], name4[CNLENGTH], name5[CNLENGTH], name6[CNLENGTH], 
    name7[CNLENGTH], name71[CNLENGTH], name72[CNLENGTH], name73[CNLENGTH],
  ...
  /*else
    ...
    fprintf(fp_out, "set output \"Nlogist.ps\" \n");
    }
  */
  ...
  if (flag1 == 1 && flag2 == 1)
    {
      fprintf(fp_out, "set title 'Nested Logistic Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
    ...
    fprintf(fp_out, "set title 'Nested Logistic Model' \n");
  ...
  fprintf(fp_out, "f(x)= (x > 0 ? ( alpha+theta1*fixd+(1-alpha-theta1*fixd)/(1+exp(-beta-theta2*fixd-rho*log(x))) ) : (alpha + theta1*fixd) )\n");
  ...
  fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Nested Logistic',\\", 
	  xmin, xmax, ybottom, ytop);
  fprintf(fp_out, "\n     '-' using 1:2:3:4 notitle with errorbars ls 16");

  if (flag1 == 1 && flag2 == 1) fprintf(fp_out, ",\\");
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=nlogist_plot.o EXEBASE=05nlogist" */
/* End: */
