...
/*********************************************************************
*
*  Poly_plot.c - an ANSI C program that reads the output file 
...
char  *argv[];
{
	...
	fprintf(fp_out, "set output \"Poly.ps\" \n");
	}
	*/
	...
	if (flag1 == 1) 
	{
	  if(nparm > 4)
	    {
	      if (rtype != 3) {
		fprintf(fp_out, "set title 'Polynomial Model, with BMR of %lg %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			bmrf, pcRiskType, conf);
	      } else {
		/* Point Estimate */
		fprintf(fp_out, "set title 'Polynomial Model, with Point Estimate BMR of %lg for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			bmrf, conf);
	      }
	    ...
	    {
	      if (rtype != 3) {
		fprintf(fp_out, "set title 'Linear Model, with BMR of %lg %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			bmrf, pcRiskType, conf);
	      } else {
		/* Point Estimate */
		fprintf(fp_out, "set title 'Linear Model, with Point Estimate BMR of %lg for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			bmrf, conf);
	      }
			fprintf(fp_out, "rl = %lg \n", rsl);
			fprintf(fp_out, "bmd = %lg \n", bmd);          
	...
	{
	if(nparm > 4)
	fprintf(fp_out, "set title 'Polynomial Model' \n");
	else
	fprintf(fp_out, "set title 'Linear Model' \n");
	}
	*/
	...
	fprintf(fp_out, "f(x)= b0 + (");
	for (i=1; i<=nparm-3; i++)
	{
		fprintf(fp_out, "b%d*(x**%d)", i, i);
		if (i < nparm-3)
			fprintf(fp_out, "+");
	}
	fprintf(fp_out, ") \n");
	...
	{
		if(nparm > 4)
			fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Polynomial',\\", 
			xmin, xmax, ybottom, ytop);
		else
			fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Linear',\\", 
			xmin, xmax, ybottom, ytop);
