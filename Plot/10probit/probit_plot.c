...
/*********************************************************************
*
*  Probit_plot.c - an ANSI C program that reads the output file 
*                   from Probit.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
*
**********************************************************************
* Updated for logtrans
**********************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  double  background, inter, slope, conf;
  ...
  if (argv[2] == NULL)
  ...
  else
    {
      ...
      fprintf(fp_out, "set output \"Probit.ps\" \n");
    }
  ...
  if (flag1 == 1 && flag2 == 1)
    {
		if(logtrans == 1)
		  fprintf(fp_out, "set title 'LogProbit Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			  effect*100., pcRiskType, conf);
		else
		  fprintf(fp_out, "set title 'Probit Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
			  effect*100., pcRiskType, conf);
  ...
  {
		if(logtrans == 1)
			fprintf(fp_out, "set title 'LogProbit Model' \n");
		else
			fprintf(fp_out, "set title 'Probit Model' \n");
  }
  ...
  if(logtrans == 0)
    fprintf(fp_out, "f(x)= back + (x>0?( (1- back)*(norm(inter + slope * x) )): 1/0) \n");
  else
    fprintf(fp_out, "f(x)= back + (x>0?( (1- back)*(norm(inter + slope * log(x)) )): 0) \n");
  ...
  if(logtrans == 1)
	    fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'LogProbit',\\", 
	  xmin>0?0:xmin, xmax, ybottom, ytop);
  else
	    fprintf(fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'Probit',\\", 
	  xmin>0?0:xmin, xmax, ybottom, ytop);
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=probit_plot.o EXEBASE=10probit" */
/* End: */
