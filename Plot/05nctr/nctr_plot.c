...
/*********************************************************************
*
*  NCTR_plot.c - an ANSI C program that reads the output file 
*                   from NCTR.C and then produces a script file for
*					GNU plot.
*  April, 1998
*
**********************************************************************/
...
main (int argc, char *argv[])
{
	...
	char name1[CNLENGTH], name2[CNLENGTH], name3[CNLENGTH], name4[CNLENGTH], name5[CNLENGTH], name6[CNLENGTH],
		name7[CNLENGTH], name71[CNLENGTH], name72[CNLENGTH], name73[CNLENGTH], name8[CNLENGTH], name9[CNLENGTH],
		name10[CNLENGTH], name11[CNLENGTH], name12[CNLENGTH], name13[CNLENGTH], name14[CNLENGTH], name15[CNLENGTH],
		name16[CNLENGTH], name17[CNLENGTH], name18[CNLENGTH], name19[CNLENGTH];
	...
	double alpha, beta, theta1, theta2, rho, fixedsize, conf;
	...
	fscanf (fp_in, "%s %lg", name5, &alpha);
	fscanf (fp_in, "%s %lg", name6, &beta);
	fscanf (fp_in, "%s %lg", name7, &theta1);
	fscanf (fp_in, "%s %lg", name71, &theta2);
	fscanf (fp_in, "%s %lg", name72, &rho);
	fscanf (fp_in, "%s %lg", name73, &fixedsize);
	...
	else
	{
		...
		fprintf (fp_out, "set output \"nctr.ps\" \n");
	}
	...
	if (flag1 == 1 && flag2 == 1)
	{
	  fprintf (fp_out, "set title 'NCTR Model, with BMR of %lg%% %s for the BMD and %lg Lower Confidence Limit for the BMDL' \n",
		...
		fprintf (fp_out, "set title 'NCTR Model' \n");

	fprintf (fp_out, "alpha = %g \n", alpha);
	fprintf (fp_out, "beta = %g \n", beta);
	fprintf (fp_out, "theta1 = %g \n", theta1);
	fprintf (fp_out, "theta2 = %g \n", theta2);
	fprintf (fp_out, "rho = %g \n", rho);
	fprintf (fp_out, "fixdsize = %g \n", fixedsize);
	fprintf (fp_out, "f(x)= 1 - ( exp( -(alpha +(theta1 *fixdsize)) - (beta + (theta2 * fixdsize)) * (x ** rho) ) )\n");
	xrange = xmax - xmin;
	yrange = ymax - ymin;
	...
	fprintf (fp_out, "plot [x=%f:%f] [%f:%f] f(x) title 'NCTR',\\",
...
/* Local Variables: */
/* compile-command: "make -f ../rMakefile OBJS=nctr_plot.o EXEBASE=05nctr" */
/* End: */
