...
char Version_no[]="Multistage Cancer Model. (Version: 1.10;  Date: 02/28/2013)";
/*
char Version_no[]="Multistage Cancer Model. (Version: 1.9;  Date: 05/26/2010)";
char Version_no[]="Multistage Cancer Model. (Version: 1.8;  Date: 10/28/2009)";
char Version_no[]="Multistage Cancer Model. (Version: 1.7;  Date: 05/16/2008)";
char Version_no[]="Multistage Cancer Model. (Version: 1.6;  Date: 03/11/2008)";
char Version_no[]="Multistage Cancer Model. (Version: 1.5;  Date: 02/20/2007)";
char Version_no[]="Multistage Cancer Model. (Version: 1.3;  Date: 09/07/2006)";
char Version_no[] = "Cancer Model. $Revision: 1.0 $ $Date: 2004/10/03 11:51:21 $";
*/
/*******************************************************************
*
* Cancer.C - a ANSI C program for Cancer model fitting with/without
...
* Version Number: 1.0
* Modified By: Qun He
* Modified Date: 10/03/2004 
* Reason: Create cancer model from Multistage.
*
* Version Number: 1.1
* Modified By: Micheal Ferree
* Modified Date: 8/08/2005
* Reason: Changed to create BMDU.
*
* Version Number:  1.2
* Modified By:  Woodrow Setzer
* Modified Date:  10/20/2005
* Reason:  Free all allocated memory; re-work the use of bind
*          to track parameter estimates on boundaries.
*         Changed Chi^2 Res to Scaled Res for consistency with other quantal
*           models
*         Fixed scaled residuals and so that
*           divisor is standard error, not variance.
*
* Version Number:  1.3
* Modified By:  Woodrow Setzer
* Modified Date:  09/07/2006
* Reason:  Constrain BMDU > BMD
*          Make search work on the log(BMD) scale
*          Clean up code specifying inequality constraints
*
* Version Number:  1.4
...
*		Changed all occurence of "Cancer" to "Multistage Cancer".
*
* Version Number:  1.5
* Modified By: Woodrow Setzer
* Date: 2/20/2007
* Reason: Incremented version number to indicate changed compilation
*         options
...
extern void loadcommbloc_(long int *ndoses, double *xmax,
						  long int nanimals[],double doses[],
						  long int affect[], long int *polyord,
...
void Cancer_fit(int nparm, double p[], double gtol,
				int *iter, double *fret);
void Cancer_BMD(int nparm, double p[], double gtol, int *iter, double xlk,
				double Rlevel[], double Bmdl[],double Bmdu[],double *BMD);
void Cancer_vcv(int nparm, int Spec[], double p[], double **vcv);
...
double LR;     /* likelihood ratio used in call to Cancer_BMD */
...
/****************************************************************
** main--main function used to call Cancer mode fitting program.
...
int main (int argc, char *argv[])
{
	...
	int      junk;             /* argument passed in call to Cancer_BMD */
	...
	*********************************************************************/
	...
	/* ------------ Set risk type to extra risk, regardless of what the input says: ---------- */
	/* bmdparm.risk = 0; */
	/* --------------------------------------------------------------------------------------- */
	...
	if (Nobs < (nparm-nparm_known))
	{
		...
		//FreeUp_mem(Parms, varsum, anasum, vcv);
		//ERRORPRT("Observation # < parameter # for Multistage Cancer model.");
		Warning("Observation # < parameter # for Multistage Cancer model.");
	}
	...
	/* fitting Cancer model and output parameter estimators */
	fflush(fp_out);
	if(nparm_known<nparm)
		Cancer_fit(nparm, Parms, EPS, &iter, &xlk);
	...
	Cancer_vcv(nparm,Spec,Parms,vcv);
	...
	OP_ParmsE(nparm,Spec,parameters,Parm_name,vcv_adj, bounded, bmdparm.level, 0);
	fflush(fp_out);
	/* free memory */
	...
	if (bmdose==Yes) /* if BMD calculation was selected */
	{
		...
		Cancer_BMD (nparm, Parms, EPS, &junk, xlk, Rlevel, Bmdl, Bmdu, &BMD);
...
/******************************************************************
**Cancer_vcv -- used to compute the info matrix for Cancer model.
...
/*********************************************************************
*Cancer_fit -- Used to "prepare" the data for further computation,
*            i.e. compute the extern variables, give the initial
*            parameters, etc. THEN fit the Cancer model.
...
void Cancer_fit(int nparm, double p[], double gtol,
				int *iter, double *fret)
{
	long int *affect, *nanim, *Spec2, *bind, ndegree, optite, nresm;
	...
	for(i = 1; i <= Nobs; i++)
	{
		nanim[i-1] = (int) (Yp[i] + Yn[i]); /* # of animals at ith dose level */
		doses[i-1] = Xi[i]/xmax;                 /* dose levels; rescaled to: 0 <= Dose <= 1  */
		affect[i-1] = (int) (Yp[i]);        /* # of affected animals at ith dose level */
	} /* end for */
...
/***************************************************************************
* Cancer_BMD -- Used to calculate the BMD and BMDL for Cancer model.
*  external: bmdparm
*  input:
*   nparm is the number of parameters
*   p[] is the vector of fitted parameters
...
{ 	/* ck , BMD_lk and LR are calculated in Cancer_BMD() */
	...
	/* in Cancer.c  Make sure right risk is going to GETCL: */
